...
// TODO: verificar se a foto também foi excluída
describe('Delete Package (E2E)', () => {
  ...
  test('[DELETE] /packages/:id', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete(`/packages/${pkg.id.toString()}`)
    ...
    expect(packageOnDatabase).toBeFalsy()
  })
  ...
  test('it should receive 403 when user does not have an admin role', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete('/packages/non-existent-id')
      .set('Authorization', `Bearer ${accessToken}`)
